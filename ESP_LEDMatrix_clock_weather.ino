...
// =======================================================================
// CHANGEZ VOS BROCHES SI NECESSAIRE ICI:
// =======================================================================
// for ESP-01 module
//#define DIN_PIN 15 // D4
...
void setup()
{
  ...
  sendCmdAll(CMD_INTENSITY,1); // Reglage de l'intensite de 0 a 15 des LED de la matrice.
  WiFi.mode(WIFI_STA);
  Serial.begin(115200);
  delay(1000);
  Serial.println("\n");
  ...
  {
   printStringWithShift("Configure le reseau wifi grace au SSID <Horlorge>",15);
  delay(5000);
   printStringWithShift("Pass : 0123456789",15);
  delay(5000);
  }
  else
  {
  printStringWithShift("Salut ",15);
  delay(5000);
  }
...
void loop()
{
  ...
  if(millis()-clkTime > 30000 && !del && dots) { // horloge pendant 15s, puis défile pendant environ 30s
...
if (client.connect(weatherHost, 80)) {
    ...
    Serial.println("connection no ok");
    return;
  }
  String line;
  int repeatCounter = 0;
...
line += c;
 ...
 // weatherString = "         Temp: " + String(temp,1) + deg + "C (" + String(tempMin,1) + deg + "-" + String(tempMax,1) + deg + ")  ";
  ...
  weatherString += "  Humidite: " + String(humidity) + "%                    ";
  //weatherString += "  Pression: " + String(pressure) + "hPa  ";
  //weatherString += "  Nuage: " + String(clouds) + "%  ";
  //weatherString += "  Vent: " + String(windSpeed,1) + "m/s                 ";
}
